...
const queryParams = reactive({ page: 1, size: 10, type: 1, keyword: '' })

const columns = [
    { label: 'ID', prop: 'id' }, { label: '创建时间', prop: 'createdAt' },
    { label: '入库单号', prop: 'recordNo' }, { label: '商品名称', prop: 'productName' },
...
const handleAdd = () => router.push('/dashboard/inventory/stock-in-create')
const handleView = (row: any) => router.push(`/dashboard/inventory/stock-in-detail?id=${row.id}`)
const handleEdit = (row: any) => router.push(`/dashboard/inventory/stock-in-create?id=${row.id}`)
...
<template>
    <el-card>
        <div class="header-actions">
            <div class="left">
                <el-input v-model="queryParams.keyword" placeholder="入库单号/商品" style="width:200px" clearable @clear="handleSearch" />
                <el-button type="primary" :icon="Search" @click="handleSearch" style="margin-left:10px">搜索</el-button>
            </div>
            <div class="right" style="display:flex;gap:10px;align-items:center">
                <BatchToolbar :selected-ids="selectedIds" :columns="columns" :table-data="tableData" entity-name="入库单" @batch-delete="handleBatchDelete" />
