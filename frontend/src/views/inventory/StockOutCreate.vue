...
import { stockOut, getAllProducts, getWarehouses } from '../../api/inventory'
...
const onProductSelect = (item: any, productId: number) => {
    ...
    if (p) {
        ...
        item.unit = p.unit || ''
        item.unitPrice = p.salePrice || p.price || 0
    }
}
...
const handleSubmit = async () => {
    ...
    try {
        ...
        router.push('/dashboard/inventory/stock-out-list')
...
<template>
    <el-card>
        <template #header><span>新增出库单</span></template>
        ...
        <el-table :data="items" border style="width:100%;margin-top:10px">
            ...
            <el-table-column label="销售金额" width="110">
