...
const handlePrint = () => {
    ...
    const rows = items.map((item, idx) =>
        ...
        <td>¥${item.costPrice || item.price || 0}</td><td>¥${item.amount || 0}</td>
        <td>¥${item.costAmount || item.amount || 0}</td><td></td><td></td></tr>`).join('')
    const html = `<html><head><title>采购发票</title>
        ...
        <div class="info">
            ...
            <span>采购单号: ${o.orderNo}</span>
            <span>供应商: ${o.supplierName || o.supplierId || ''}</span>
            <span>业务员: ${o.salesman || ''}</span>
        </div>
        <table><tr><th>序号</th><th>商品名称</th><th>采购数量</th><th>成本单价</th>
        <th>进货金额</th><th>实际付款</th><th>折扣</th><th>欠款</th></tr>
        ${rows}
        <tr><td colspan="4" style="text-align:right;font-weight:bold">合计</td>
        <td>¥${o.dealAmount || o.totalAmount || 0}</td>
        <td>¥${o.totalCost || 0}</td><td>${o.discount || 0}%</td>
        ...
        <div v-if="order">
            ...
            <el-table :data="order.items" border>
                ...
                <el-table-column label="成本单价" width="110">
                    <template #default="{ row }">¥{{ row.costPrice || row.price || 0 }}</template>
                </el-table-column>
                <el-table-column label="进货金额" width="110">
                    <template #default="{ row }">¥{{ row.amount || 0 }}</template>
                </el-table-column>
                <el-table-column label="实际付款" width="110">
